...
func NewGenerator(gc GeneratorConfig) (*generator, error) {
	...
	for _, swarmNodeEndpoint := range swarmNodes {
		log.Printf("Attempting to connect to swarm node: %s", swarmNodeEndpoint)
		...
		if err != nil {
			log.Printf("Unable to create docker client for endpoint %s: %s", swarmNodeEndpoint, err)
			continue
		}
	...
	for _, endpoint := range g.SwarmNodes {
		go func(endpoint string) {
			...
			for {
				if client == nil {
					var err error
					log.Printf("Attempting to connect to Docker endpoint: %s", endpoint)
					...
					if err != nil {
						log.Printf("Unable to connect to Docker endpoint %s: %s", endpoint, err)
					...
					if err != nil && err != docker.ErrListenerAlreadyExists {
						log.Printf("Error registering Docker event listener on endpoint %s: %s", endpoint, err)
						client = nil
						listenerChan = nil
						time.Sleep(10 * time.Second)
						continue
					}
					log.Printf("Watching Docker events on endpoint: %s", endpoint)
					eventChan <- nil
				}
				select {
				case event, ok := <-listenerChan:
					if !ok {
						log.Printf("Docker daemon connection interrupted on endpoint: %s", endpoint)
					...
					if event.Status == "start" || event.Status == "stop" || event.Status == "die" || strings.Index(event.Status, "health_status:") != -1 {
						log.Printf("Received event %s for container %s on endpoint %s", event.Status, event.ID[:12], endpoint)
						eventChan <- event
					}
				case <-time.After(10 * time.Second):
					err := client.Ping()
					if err != nil {
						log.Printf("Unable to ping Docker daemon on endpoint %s: %s", endpoint, err)
						client.RemoveEventListener(listenerChan)
						client = nil
						listenerChan = nil
					}
				case <-done:
					log.Printf("Done signal received for endpoint: %s", endpoint)
					client.RemoveEventListener(listenerChan)
					client = nil
					listenerChan = nil
					return
